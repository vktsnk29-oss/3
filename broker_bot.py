...
GEOCODE_UA = os.getenv("GEOCODE_UA", "tg-broker-bot/inline-geo/2.0 (contact: set-your-email@example.com)")
...
async def on_imenu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "home":
        ...
        await q.message.reply_text(
            ...
            "4) Адрес: если вы **на объекте** — нажмите большую кнопку снизу «📍 Отправить моё местоположение». "
            "Или выберите вариант «✏️ Ввести адрес текстом» и напишите адрес.\n"
...
async def desc_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Адрес объекта.\n\nЕсли вы **на месте**, нажмите большую кнопку снизу "
        "«📍 Отправить моё местоположение» — это проще.\n"
        "Если вы не на месте, выберите «✏️ Ввести адрес текстом». Кнопки исчезнут после выбора.",
        reply_markup=kb
    )
    return LOC_CHOICE

async def on_loc_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Either location payload or the "enter address text" signal
    if update.message.location:
        ...
        # Might be any text; if it's the explicit prompt, proceed to text address
